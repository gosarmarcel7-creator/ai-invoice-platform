...
const HOW_IT_WORKS = [
  { step: "01", title: "Upload", body: "Drop any invoice format — PDF, image, or text." },
  { step: "02", title: "AI Extracts", body: "Mistral AI pulls vendor, amounts, dates, and line items." },
  { step: "03", title: "Review", body: "Validate in the review queue, then approve with one click." },
];
  ...
  return (
    <div className="w-full space-y-6">
      <AgPageHeader
        ...
        subtitle="Upload any invoice format. AI extracts all key data automatically."
      />
                ...
                <AgGlassCard className="overflow-hidden">
                  ...
                  <ul className="max-h-64 divide-y divide-white/5 overflow-y-auto">
                    {queue.map((item) => (
                      <li key={item.id} className="group flex items-center gap-3 px-5 py-3">
                        <span className="grid h-8 w-8 place-items-center rounded-lg border border-[var(--ag-border)] text-[10px] font-black">
                          {ext(item.file.name)}
                        </span>
                        ...
                        {item.status === "pending" && (
                          <button type="button" onClick={(e) => { e.stopPropagation(); remove(item.id); }} className="opacity-0 group-hover:opacity-100">
                  ...
                  <div className="flex items-center justify-between border-t border-[var(--ag-border)] px-5 py-3.5">
                    <p className="text-xs text-[var(--ag-text-tertiary)]">
                      {allDone ? "All files uploaded" : `${pendingCount} ready to upload`}
                    </p>
                    {allDone ? (
                      <button type="button" onClick={() => router.push("/review")} className="ag-btn-primary">
                        View Review Queue <ArrowRight className="h-3.5 w-3.5" />
                      </button>
                    ) : (
                      <button type="button" onClick={upload} disabled={uploading || !pendingCount} className="ag-btn-primary">
                        {uploading ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Uploading…</> : <><UploadCloud className="h-3.5 w-3.5" /> Start AI Extraction</>}
                      </button>
                    )}
                  </div>
                </AgGlassCard>
