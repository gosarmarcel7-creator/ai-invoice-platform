    ...
    <div className="w-full space-y-5">
      ...
      <motion.div {...fade} className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            ...
            className="rounded-xl border border-[var(--ag-border)] bg-[var(--ag-surface-glass)] p-2 text-[var(--ag-text-secondary)] transition-all hover:border-[var(--ag-border-strong)] hover:text-white"
          >
      ...
      <motion.div {...fade} transition={{ delay: 0.1 }}>
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
          {/* Left: raw document */}
          <HoverCard className="flex flex-col overflow-hidden" style={{ minHeight: 600 }}>
            <div className="flex items-center gap-2 border-b border-[var(--ag-border)] bg-[var(--ag-surface-glass)] px-5 py-3.5">
            ...
            <div className="flex-1 overflow-auto p-5">
              {invoice?.raw_text ? (
                ...
                <div className="flex h-full flex-col items-center justify-center py-16 text-center">
                  <FileText className="mb-3 h-8 w-8 text-[var(--ag-text-disabled)]" />
                  <p className="text-sm font-medium text-[var(--ag-text-secondary)]">No text preview available</p>
                  <p className="mt-1 text-xs text-[var(--ag-text-tertiary)]">Original document content not extracted</p>
                </div>
              )}
            </div>
          </HoverCard>
          ...
          <HoverCard className="flex flex-col overflow-hidden" style={{ minHeight: 600 }}>
            <div className="flex items-center justify-between border-b border-[var(--ag-border)] bg-[var(--ag-surface-glass)] px-5 py-3.5">
              <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-[var(--ag-text-tertiary)]">$</span>
                      <input 
              ...
              <motion.div {...fade} transition={{ delay: 0.25 }}>
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    {!isReadOnly && (
                      <button onClick={addLineItem} className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--ag-text-secondary)] transition-colors hover:text-white">
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-[var(--ag-border)] bg-[var(--ag-surface-glass)]">
                          <th className="px-3 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-[var(--ag-text-tertiary)]">Description</th>
                          <th className="w-16 px-3 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-[var(--ag-text-tertiary)]">Qty</th>
                          <th className="w-24 px-3 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-[var(--ag-text-tertiary)]">Unit ($)</th>
                          <th className="w-24 px-3 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-[var(--ag-text-tertiary)]">Total</th>
                      ...
                      <tbody className="divide-y divide-[var(--ag-border)]">
                        {(!form.line_items || form.line_items.length === 0) ? (
                          <tr>
                            <td colSpan={isReadOnly ? 4 : 5} className="px-3 py-8 text-center text-xs text-[var(--ag-text-tertiary)]">
                              No line items — {isReadOnly ? "none were extracted." : 'click "Add row" to add one'}
                            </td>
                          </tr>
                        ) : (
                          form.line_items.map((item, i) => (
                            <tr key={i} className="group transition-colors hover:bg-[var(--ag-surface-glass)]">
                              <td className="px-2 py-2">
                                <input
                                  className="w-full border-b border-transparent bg-transparent py-0.5 text-sm text-white outline-none transition-colors placeholder:text-[var(--ag-text-disabled)] focus:border-[var(--ag-primary-400)]"
                              ...
                              <td className="px-2 py-2">
                                <input className="w-full border-b border-transparent bg-transparent py-0.5 text-sm text-white outline-none transition-colors focus:border-[var(--ag-primary-400)]" type="number" value={item.quantity ?? ""} onChange={(e) => setLineItem(i, "quantity", parseFloat(e.target.value) || 0)} disabled={isReadOnly} />
                              </td>
                              <td className="px-2 py-2">
                                <input className="w-full border-b border-transparent bg-transparent py-0.5 text-sm text-white outline-none transition-colors focus:border-[var(--ag-primary-400)]" type="number" step="0.01" value={item.unit_price ?? ""} onChange={(e) => setLineItem(i, "unit_price", parseFloat(e.target.value) || 0)} disabled={isReadOnly} />
                              </td>
                              <td className="px-2 py-2">
                                <span className="tabnum text-sm font-semibold text-[var(--ag-text-secondary)]">{formatCurrency(item.total_price)}</span>
                              </td>
                              {!isReadOnly && (
                                <td className="px-2 py-2">
                                  <button onClick={() => removeLineItem(i)} className="text-[var(--ag-text-tertiary)] opacity-0 transition-opacity hover:text-rose-400 group-hover:opacity-100">
                      ...
                      {(form.line_items?.length ?? 0) > 0 && (
                        <tfoot>
                          <tr className="border-t border-[var(--ag-border)] bg-[var(--ag-surface-glass)]">
                            <td colSpan={3} className="px-3 py-2.5 text-right text-xs font-bold uppercase tracking-wider text-[var(--ag-text-tertiary)]">
                              Subtotal
                            </td>
